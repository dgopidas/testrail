...
LABEL vendor="TestRail" \
      maintainer="Divya Gopidas" \
      email="dgopidas@genixventures.com" \
...
COPY testrail-6.1.1.1020-ion71.zip /tmp/

RUN \
  mkdir -p /var/www/testrail && \
  mkdir -p /opt/testrail/attachments /opt/testrail/reports /opt/testrail/logs /opt/testrail/audit && \
  unzip /tmp/testrail-6.1.1.1020-ion71.zip -d /var/www/ && \
  rm /tmp/testrail-6.1.1.1020-ion71.zip && \
